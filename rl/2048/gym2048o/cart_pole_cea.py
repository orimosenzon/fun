...
class CPCrossEntropyAgent: 
    ...
    def __init__(self, env):
        self.env = env
        self.obs_size = 4 
        self.net = Net(self.obs_size, self.HIDDEN_SIZE, env.action_space.n) 
        self.episodes = [] 
        self.optimizer = torch.optim.Adam(params=self.net.parameters(), lr=0.01)
        self.loss_fn = nn.CrossEntropyLoss()
        self.sm = nn.Softmax() 
        self.writer = SummaryWriter(comment='__CP Cross Entropy Agent')
