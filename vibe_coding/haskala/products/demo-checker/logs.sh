#!/usr/bin/env bash
# Read demo-checker's Cloud Run logs — the trace of what happened when a
...
#   DEMO_CHECKER_LOG_LEVEL=DEBUG ./deploy.sh "temporarily verbose logging"
# and plain ./deploy.sh puts it back to INFO.
#
# For a quick look without a redeploy:
#   gcloud run services update demo-checker --region europe-west1 \
