"""core.py — ליבת הבדיקה של math-checker, ללא Flask.

כל צרכן — ה-web UI (app.py), ה-CLI (check.py) וה-poller של Classroom —
קורא לאותו pipeline: רינדור עמודים → תיקון אוריינטציה (Haiku) → ניתוח
הוליסטי (Opus) → JSON מובנה. אין כאן state של שרת (JOBS/queues) בכוונה.
"""
...
def count_pages(data: bytes, ext: str) -> int:
    """מספר העמודים בלי לרנדר אף אחד (זול — קורא רק את מבנה ה-PDF)."""
    if ext != ".pdf":
        return 1
...
def iter_file_pages(data: bytes, ext: str):
    """כמו file_to_pages אבל עצל: מרנדר עמוד אחד בכל פעם, כך שבשיא מוחזק
    בזיכרון render מלא (200 DPI) של עמוד יחיד ולא של כל המסמך."""
    if ext != ".pdf":
...
def process_stream(file_bytes: bytes, ext: str, auto_orient: bool,
    ...
    keep_imgs=False (CLI/poller): לא צוברים את תמונות העמודים לטובת re-analysis —
    חוסך ~10MB לעמוד בזיכרון. ה-UI צריך אותן (סיבוב ידני), ה-batch לא."""
