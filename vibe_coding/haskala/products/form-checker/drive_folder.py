...
def write_report(drive, folder_id: str, name: str, docx_bytes: bytes) -> str:
    """Upload a .docx and have Drive convert it to a Google Doc. Returns the id.

    Converted rather than left as a .docx because the form promises Google Docs,
    and because the point of putting results in the teacher's folder is that
    they can open one on a phone without downloading anything.
    """
    media = MediaIoBaseUpload(io.BytesIO(docx_bytes), mimetype=DOCX_MIME,
                              resumable=False)
    created = drive.files().create(
        body={"name": name, "parents": [folder_id], "mimeType": GDOC_MIME},
        media_body=media, fields="id, webViewLink",
        **{"supportsAllDrives": True},
    ).execute()
    log.info("wrote report %r (id=%s, %.0f KB source)",
             name, created["id"], len(docx_bytes) / 1024)
    return created["id"]
