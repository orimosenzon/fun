#!/usr/bin/env bash
# Deploy form-checker to Cloud Run, always from a committed-and-pushed state.
...
#   4. gcloud run deploy form-checker --source .
#
...
gcloud run deploy "$SERVICE" \
    ...
    --set-env-vars="^|^RESPONSES_SHEET_ID=${RESPONSES_SHEET_ID}|WORKSPACE_SUBJECT=${WORKSPACE_SUBJECT:-ori@bdika.net}|SHARE_WITH=${SHARE_WITH:-${WORKSPACE_SUBJECT:-ori@bdika.net}}|OUTPUT_FOLDER_NAME=${OUTPUT_FOLDER_NAME:-Bdika}|FALLBACK_RESULTS_FOLDER_ID=${FALLBACK_RESULTS_FOLDER_ID:-}|FORM_CHECKER_ALLOWED_DOMAINS=${FORM_CHECKER_ALLOWED_DOMAINS:-}|MAX_WORKS_PER_FOLDER=${MAX_WORKS_PER_FOLDER:-3}|MAX_FILES_PER_RESPONSE=${MAX_FILES_PER_RESPONSE:-60}|MAX_PAGES_PER_RESPONSE=${MAX_PAGES_PER_RESPONSE:-40}|MAX_ROWS_PER_RUN=${MAX_ROWS_PER_RUN:-5}|AZURE_OPENAI_ENDPOINT=https://haskala-foundry-resource.openai.azure.com/openai/v1|AZURE_OPENAI_DEPLOYMENT=gpt-4.1-mini|LOG_LEVEL=${FORM_CHECKER_LOG_LEVEL:-INFO}"

echo "✅ deployed."
echo "   First deploy only: run ./setup_infra.sh to wire the Cloud Scheduler trigger."
if [[ -z "${FORM_CHECKER_ALLOWED_DOMAINS:-}" ]]; then
    echo "⚠️  FORM_CHECKER_ALLOWED_DOMAINS is empty — anyone with the form link can"
    echo "   spend model budget. Set it before the link circulates."
fi
