"""main.py — math-form-checker: grade a shared folder of maths exams via a Form.

A teacher fills in a Google Form — pasting their marking scheme and a link to a
Drive folder of scanned work — and the reports appear back inside that same
...
student work in the linked folder with haskala_grading.math_core, and writes one
Word document per file back into the folder.

RELATIONSHIP TO form-checker
────────────────────────────
This is products/form-checker with the grading engine swapped. Everything about
Google, the ledger, admission control and the guardrails is the same design and
was copied deliberately rather than shared, because the two services are
deployed and rate-limited separately. Three differences are load-bearing:

  • The engine is math_core (holistic whole-page analysis, LaTeX steps, per
    problem points) rather than core (line-by-line OCR against a Ministry
    rubric). See checker.py.
  • Reports stay as .docx instead of being converted to Google Docs — Drive's
    importer reverses LaTeX inside an RTL document. See drive_folder.write_report.
  • The Firestore collections are math_form_checker_* . Sharing form-checker's
    would make each service treat the other's rows as already handled, and the
    symptom would be submissions silently never grading.
...
#
# The dominant cost is per page: a Haiku rotation call plus one holistic
# analysis call, roughly $0.03 per page on Claude Sonnet 5 at introductory
# rates (~$0.05 at standard, ~2.5x that on Opus 4.8). So the page cap, not the
# file cap, is the one that actually bounds the bill.
#
# TIME, NOT MONEY, IS THE TIGHTER CONSTRAINT HERE — and this is the main way
# the maths guardrails differ from form-checker's.
#
# Measured 2026-08-05 on a real geometry page: 7s to detect rotation, 46s to
# analyse. Call it ~55s per page against form-checker's ~25s per *work*. The
# defaults below are sized so the worst case a single invocation can reach —
# MAX_ROWS_PER_RUN x MAX_PAGES_PER_RESPONSE x 55s — stays inside both the
# Cloud Run request timeout (3600s, set in deploy.sh) and the Scheduler attempt
# deadline (1800s, set in setup_infra.sh). At 1 x 12 x 55s that is ~11 minutes.
#
# Raising MAX_PAGES_PER_RESPONSE or MAX_ROWS_PER_RUN without raising those two
# deadlines does not fail loudly: the run is killed mid-folder, the row stays
# claimed until _STALE_CLAIM_MINUTES expires, and the teacher waits for a report
...
MAX_PAGES_PER_RESPONSE = int(os.environ.get("MAX_PAGES_PER_RESPONSE", "12"))

# Rows processed in one invocation. A backlog is drained over several polls
# rather than in one run that blows the Scheduler's attempt deadline and gets
# retried from the top — the ledger makes the partial progress safe.
#
# One, not form-checker's five: a maths row can legitimately occupy eleven
# minutes on its own (see above), so draining several in one invocation is how
# the deadline gets blown. The poll runs every five minutes, so a queue of rows
# still clears steadily — just one row per poll.
MAX_ROWS_PER_RUN = int(os.environ.get("MAX_ROWS_PER_RUN", "1"))
...
#   MATH_FORM_CHECKER_ALLOWED_DOMAINS — comma-separated, e.g. "bdika.net,shamir.org.il"
#   MATH_FORM_CHECKER_PASSPHRASE — a short code on the form, shared with the teachers
ALLOWED_DOMAINS = [d.strip().lower() for d in
                   os.environ.get("MATH_FORM_CHECKER_ALLOWED_DOMAINS", "").split(",") if d.strip()]
PASSPHRASE = os.environ.get("MATH_FORM_CHECKER_PASSPHRASE", "").strip()
...
def file_key(params: dict, f: dict) -> str:
    ...
    material = "|".join([
        ...
        f.get("id", ""),
        f.get("modifiedTime", ""),
        params.get("rubric_text") or "",
        params.get("model_key") or "",
    ])
    return hashlib.sha256(material.encode("utf-8")).hexdigest()[:32]


def _file_already_graded(key: str) -> bool:
    snap = _firestore().collection("math_form_checker_files").document(key).get()
...
def _mark_file_done(key: str, folder_id: str, file_id: str, report_id: str = ""):
    _firestore().collection("math_form_checker_files").document(key).set({
        "status": "done",
        "folder_id": folder_id,
        "file_id": file_id,
        "report_id": report_id,
...
def _try_claim_row(key: str) -> bool:
    ...
    doc_ref = _firestore().collection("math_form_checker_responses").document(key)
...
def _mark_done(key: str, outcome: str = "graded"):
    _firestore().collection("math_form_checker_responses").document(key).set({
        "status": "done",
        "outcome": outcome,
        "completed_at": datetime.datetime.now(datetime.timezone.utc),
    }, merge=True)
...
def _admit(params: dict, tag: str) -> str | None:
    ...
    if ALLOWED_DOMAINS:
        ...
        if domain not in ALLOWED_DOMAINS:
            log.warning("%s rejecting %s — domain %r is not in MATH_FORM_CHECKER_ALLOWED_DOMAINS",
...
def _process_folder(drive_service, gmail_service, params, tag) -> str:
    ...
    results = {
        ...
        # the model scores by the points printed beside each problem.
        "task_known": bool(ctx.rubric_text),
    }

    for entry in pending:
        fkey = file_key(params, entry)
        try:
            docx_bytes, title = checker.check_file(entry, params, ctx, rubric, tag=tag)
...
def run_form_scan(dry_run: bool = False):
    ...
    if not ALLOWED_DOMAINS and not PASSPHRASE:
        log.warning("no MATH_FORM_CHECKER_ALLOWED_DOMAINS and no MATH_FORM_CHECKER_PASSPHRASE — "
    ...
    for row_num, row in enumerate(rows, start=2):   # +2: header is row 1
        ...
        if dry_run:
            ...
            for k in ("timestamp", "email", "teacher_name", "grade_level",
                      "school", "folder_id", "rubric_in_folder", "model_key",
                      "comments"):
                print(f"   {k:18}: {params.get(k)!r}")
            print(f"   {'rubric':18}: {form_schema.build_rubric(params)[:200]!r}")
