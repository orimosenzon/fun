...
קלט:  logs/<run>_train.jsonl  (עקומות למידה),  reports/data/eval_<name>.json  (הערכה סופית)
       reports/data/analysis_<name>.html (ניתוח מילולי), reports/data/methods.html (הסבר שלוש השיטות)
פלט:  reports/dqn_report.html, reports/ac_report.html, reports/ppo_report.html, reports/comparison.html
...
# הסדר כאן הוא סדר ההצגה בכל הדו"חות. הצבעים: DQN כחול, A2C כתום, PPO ירוק.
AGENTS = {
    ...
    "ppo": {"run": "ppo", "label": "PPO", "color": "s3", "report": "ppo_report.html", "long": "Proximal Policy Optimization (PPO)"},
}
KINDS = list(AGENTS)
DIRECTIONS = ["↑ למעלה", "→ ימינה", "↓ למטה", "← שמאלה"]
THRESHOLDS = [3000, 5000, 8000, 12000, 16000, 20000, 30000, 40000]
...
def th_cells(row):
    ...
    _, t, m = row
    return f"<td class='num'>{(f'{t / 1e6:.1f}M' if t else 'לא הושג')}</td><td class='num'>{(f'{m:.0f}' if m else '')}</td>"
...
HP_LISTS = {"dqn": DQN_HP, "ac": AC_HP, "ppo": PPO_HP}
...
<section id="summary">
<h2>תקציר</h2>
<p class="lead">{A['long']} אומן {total_min:.0f} דקות על {total_trans / 1e6:.0f} מיליון מהלכים{updates_note}. בהערכה סופית של 1,000 משחקים (בלי חקירה) הוא הגיע לניקוד ממוצע של <b>{f0(S['score_mean'])}</b>,
...
<p class="note">חומרה: {HARDWARE}. הניקוד הוא ניקוד המשחק המקורי (סכום כל האריחים שנוצרו במיזוג). הסבר מפורט על האלגוריתם, כולל המתמטיקה, בסעיף <a href="comparison.html#methods">שלוש השיטות</a> של דו"ח ההשוואה.</p>
</section>
...
"""
    html = page(f"{A['label']} לומד לשחק 2048", f"""דו"ח ביצועים: {A['long']}, אימון מקומי על GPU""",
                f"אומן ב-{dt.date.today().strftime('%d.%m.%Y')} · {total_min:.0f} דקות · {total_trans / 1e6:.0f} מיליון מעברים", toc, body, crumbs_for(kind))
...
def comparison_report(R: dict, baselines: dict):
    ...
    kpis += "".join(kpi(f"{L(k)}: מעברים", f"{R[k]['transitions'] / 1e6:.0f}", "מיליון", cls=C(k),
    ...
    for k in kinds:
        ...
        td_series.append({"label": L(k), "y": tile_dist(R[k]["eval"]["max_tiles"]), "color": C(k)})
    ...
    td_cfg = {"labels": [str(t) for t in TILE_ORDER], "series": td_series, "yTitle": "מספר משחקים", "xTitle": "האריח הגדול ביותר במשחק",
              "table": {"columns": ["אריח"] + [L(k) for k in kinds], "rows": [[str(t)] + [s["y"][i] for s in td_series] for i, t in enumerate(TILE_ORDER)]}}
    ...
    summary_rows = "".join([
        ...
        row("מעברים באימון", [f"{R[k]['transitions'] / 1e6:.1f}M" for k in kinds]),
        row("מעברים בשנייה", [f"{R[k]['sps']:,}" for k in kinds]),
        row("צעדי גרדיאנט", [(f0(R[k]["updates"]) if R[k].get("updates") else "") for k in kinds]),
    ...
    aspects = ["מה הרשת לומדת", "איך נבחרת פעולה", "חקירה", "שימוש בנתונים", "יעד הלמידה", "עדכונים", "יציבות"]
    ...
    budget_parts = [f"{L(k)} {R[k]['minutes']:.0f} דקות ({R[k]['transitions'] / 1e6:.0f} מיליון מעברים)" for k in kinds]
    replays = "".join(
    ...
    toc = [("summary", "תקציר"), ("methods", "שלוש השיטות"), ("curves", "עקומות למידה"), ("final", "ההערכה הסופית"), ("table", "טבלת השוואה"), ("replay", "משחקים"), ("discussion", "דיון")]
    body = f"""
<section id="summary">
<h2>תקציר</h2>
<p class="lead">{"שלושה" if n == 3 else "שני"} אלגוריתמים, אותה רשת, אותה סביבה, אותו תגמול, אותם 1,000 משחקי מבחן. תקציב האימון: {"; ".join(budget_parts)}.
ניקוד ממוצע: {", ".join(lead_parts)}. הגעה ל-2048: {", ".join(reach_parts)}. המנצח: <b>{L(best)}</b>.</p>
<div class="kpis cols{n}">{kpis}</div>
<p class="note">הדו"חות הנפרדים: {reports_links}. חומרה: {HARDWARE}.</p>
</section>

<section id="methods">
<h2>שלוש השיטות: מה כל אלגוריתם לומד, ואיך</h2>
{data_text("methods", "ההסבר על השיטות ייכתב בקובץ reports/data/methods.html.")}
</section>

<section id="curves">
<h2>עקומות למידה</h2>
<p>ההשוואה נעשית על שני צירים, כי לאלגוריתמים יש יחס שונה בין חישוב לנתונים. DQN מבצע 8 דגימות אימון על כל מעבר, A2C דגימה אחת, ו-PPO ארבע. לכן A2C צורך מעברים מהר בהרבה, אבל מפיק מכל מעבר פחות. ציר הזמן עונה על "מה מקבלים מדקת GPU", ציר המעברים על "כמה ניסיון צריך".</p>
...
"""
    title = " מול ".join(L(k) for k in kinds) + " ב-2048"
    html = page(title, f"השוואת ביצועים של {'שלושה' if n == 3 else 'שני'} אלגוריתמי למידת חיזוק על אותו משחק, אותה רשת ואותה חומרה, עם הסבר מפורט על כל שיטה",
