...
const SKIP_TEXT = {
  ...
  frozen: ['התמונה תקועה על פריים ישן',
    'הדפדפן לא מספק פריימים חדשים לקליפ הזה. אם הניגון חסום, לחצו על המסך פעם אחת ונסו שוב. אחרת כנראה שהקודק של הקובץ בעייתי בדפדפן הזה.'],
};
...
function wireDiagnostics() {
  ...
  engine.onAutoplayBlocked = () => {
    toast('הדפדפן חסם את הניגון. לחצו במקום כלשהו בעמוד ואז שוב על נגן.', 'err', 6000);
  };
  engine.onStuck = (clip) => {
    toast(`הווידאו של "${clip.name}" לא מתקדם. לחצו על 🩺 לדוח אבחון.`, 'err', 6000);
  };
